...
/// @notice A Gnosis Safe CompatibilityFallbackHandler that restricts isValidSignature to only succeed if an accompanying FraxGovernorOmega/FraxGovernorAlpha proposal has passed.
/// @notice Original author: Richard Meissner - <richard@gnosis.pm>
contract FraxCompatibilityFallbackHandler is DefaultCallbackHandler, ISignatureValidator {
    //keccak256(
     ...
     * @dev The only function changed from Safe's CompatibilityFallbackHandler
     * @param _data bytes32 hashed message digest signed on the behalf of address(msg.sender)
     * @dev _signature Signature byte array associated with _data, not used
    ...
    function isValidSignature(bytes memory _data, bytes memory /* _signature */) public view override returns (bytes4) {
        ...
        // FraxGovernorOmega or FraxGovernorAlpha approved
